...
/**
 * This class creates a map with the aliens plotted in the appropriate
 * locations. It is of type JPanel so it 'returns' the map in the form of a
 * panel.
 * 
...
public class MapDrawer extends JPanel {
	...
	public void paint(Graphics g) {
		...
		System.out.println("Map time: " + totalTime/1000);
	}
	...
	private void setAliens() {
		...
		averageFrequency = averageFrequency / locationsToPlot.size(); // finding
																		// the
																		// average
		...
		int scaleFactor;
		for (String point : locationsToPlot.keySet()) {
			if (locationsFromFile.keySet().contains(point)) { // only adding US
																// states
				...
				if (currentFrequency > Math.ceil(averageFrequency)
						&& currentFrequency > 2 * Math.ceil(averageFrequency)) { // if
																					// the
																					// frequency
																					// if
																					// twice
																					...
																					// average
					scaleFactor = 40;
				} else if (currentFrequency > Math.ceil(averageFrequency)) { // greater
																				// than
																				// average
					scaleFactor = 20;
				} else if (currentFrequency == Math.ceil(averageFrequency)) { // equal
																				// to
																				// average
					scaleFactor = 0;
				} else if (currentFrequency < Math.ceil(averageFrequency)) { // less
																				// than
																				// average
					scaleFactor = 0;
				}
				...
				// below we are adding a panel over each alien image allowing us
				// to register a click event on each alien
				// if a click is registered on the alien, then we will load up
				// the infoGUI
				...
				alienPanel.addMouseListener(new MouseListener() {
					...
					public void mouseClicked(MouseEvent arg0) {
						...
						InfoGUI gui = new InfoGUI(stateAbrev, stateName); // loading
																			// up
																			// the
																			// infoGUI
					}
